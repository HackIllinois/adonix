    ...
    async (req: Request, res: Response) => {
        if (!req.isAuthenticated()) {
            return res
                ...
                .send({ error: "FailedAuth" });
        }

        const device: string = (res.locals.device ??
            Constants.DEFAULT_DEVICE) as string;
        const user: GithubProfile | GoogleProfile = req.user as
            | GithubProfile
            | GoogleProfile;
        const data: ProfileData = user._json as ProfileData;
        const redirect: string =
            Constants.REDIRECT_MAPPINGS.get(device) ??
        ...
        try {
            ...
            const payload: JwtPayload = await getJwtPayloadFromProfile(
                user.provider,
                data,
            );
...
authRouter.put(
    ...
    async (req: Request, res: Response) => {
        ...
        const op: RoleOperation | undefined =
            RoleOperation[req.params.operation as keyof typeof RoleOperation];

        // No operation - fail out
        if (!op) {
            return res
                .status(Constants.BAD_REQUEST)
                .send({ error: "InvalidOperation" });
        }
...
authRouter.get(
    ...
    async (_: Request, res: Response) => {
        ...
        try {
            ...
            const newPayload: JwtPayload = await getJwtPayloadFromProfile(
                oldPayload.provider,
                data,
            );
