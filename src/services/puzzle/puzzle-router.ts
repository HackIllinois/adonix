...
puzzleRouter.post("/submit/:QID", strongJwtVerification, async (req: Request, res: Response, next: NextFunction) => {
    ...
    const qid: number = parseInt(req.params.QID as string);
...
puzzleRouter.get("/ranking", weakJwtVerification, async (_1: Request, res: Response, _2: NextFunction) => {
    const teamsRanked: PuzzleItem[] = await Models.PuzzleItem.aggregate([
        {
            $addFields: {
                trueCount: {
                    $sum: {
                        $map: {
                            input: "$problemComplete",
                            as: "complete",
                            in: { $cond: ["$complete", Config.TRUE_VALUE, Config.FALSE_VALUE] },
                        },
                    },
                },
            },
        },
        { $sort: { trueCount: -1, lastCorrect: 1 } },
    ]);

    return res.status(StatusCode.SuccessOK).send({ teamsRanked });
});
