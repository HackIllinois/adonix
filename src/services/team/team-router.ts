...
teamRouter.get(
    ...
    specification({
        ...
        tag: Tag.USER,
        role: Role.USER,
        summary: "Gets a team and its users",
        parameters: z.object({
            id: z.string(),
        }),
        responses: {
            [StatusCode.SuccessOK]: {
                description: "Team and its users",
                schema: z.object({
                    team: TeamSchema,
                    users: z.array(z.any()).openapi({ description: "List of users in the team" }),
                }),
            },
    ...
    async (req, res) => {
        ...
        const staffMembers = await Models.StaffInfo.find({ team: id, isActive: true }).populate("user");
        return res.status(StatusCode.SuccessOK).json({ team, users: staffMembers });
    },
);
